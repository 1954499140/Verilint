...
import { VerilintLinter, VerilintIssue, VerilintResult, ProjectResult } from './linter';
import { VerilintCodeLensProvider } from './codelens';

export class VerilintDiagnosticsProvider {
    private linter: VerilintLinter;
    private outputChannel: vscode.OutputChannel;
    private diagnosticCollection: vscode.DiagnosticCollection | undefined;
    private codeLensProvider: VerilintCodeLensProvider | undefined;

    constructor(linter: VerilintLinter, outputChannel: vscode.OutputChannel) {
        this.linter = linter;
    ...
    setCodeLensProvider(provider: VerilintCodeLensProvider) {
    ...
    async lintDocument(document: vscode.TextDocument): Promise<void> {
        ...
        try {
            ...
            if (this.codeLensProvider) {
                ...
                const shownIssues = result.issues.filter(issue =>
                    !(issue.severity === 'info' && !showInfo)
                );
     ...
     */
    async lintProject(document?: vscode.TextDocument): Promise<void> {
        if (!this.diagnosticCollection) {
            return;
        }
        ...
        try {
            ...
            if (document && this.codeLensProvider) {
                ...
                if (docResult) {
                    const shownIssues = docResult.issues.filter(issue =>
                        !(issue.severity === 'info' && !showInfo)
                    );
                    this.codeLensProvider.setIssues(document.uri, shownIssues);
                }
    ...
    private createDiagnostic(document: vscode.TextDocument, issue: VerilintIssue): vscode.Diagnostic {
    ...
    private createDiagnosticForFile(filePath: string, issue: VerilintIssue): vscode.Diagnostic {
        const line = Math.max(0, issue.line - 1); // Convert to 0-based
        const column = Math.max(0, issue.column - 1);
    ...
    private createDiagnosticInternal(range: vscode.Range, issue: VerilintIssue): vscode.Diagnostic {
