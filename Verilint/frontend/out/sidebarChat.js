...
class SidebarChatProvider {
    constructor(_extensionUri, outputChannel) {
        ...
        this._includeCodeContext = false;
    ...
    async _callAI(userMessage, context) {
        ...
        const apiKey = config.get('aiApiKey', 'sk-f3c469e44a4f4defadceda3cdac3e21d');
        const baseUrl = config.get('aiBaseUrl', 'https://dashscope.aliyuncs.com/compatible-mode/v1');
        const model = config.get('aiModel', 'qwen3.5-plus-2026-02-15');
        if (!apiKey) {
...
SidebarChatProvider.viewType = 'verilint.sidebarChat';
